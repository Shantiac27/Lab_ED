...
double evaluar(Arbol arbol)
{
	...
	printf("\n RESULTADO: %.2f", calcularValores(arbol.raiz));
...
void asignarMemoria(NodoA **original,int cantidad,int (*comparar)(void*,void*))
{
	...
	{	
		...
		{
			if(original[i] == NULL || original[i]->dato == NULL)
				printf("Error de memoria en original[%d]", i);
			
			if(original[j] == NULL || original[j]->dato == NULL)
				printf("Error de memoria en original[%d]", j);
...
double calcularValores(NodoA *raiz)
{
	if(!raiz) return 0;
	
	Variable *var = (Variable*) raiz->dato;
	
	if(precedencia(var) == OPERANDO)
	{
		if(var->v && *(var->v))
		{
			return **(var->v);
	...
	double izq = calcularValores(raiz->izq);
	double der = calcularValores(raiz->dch);
